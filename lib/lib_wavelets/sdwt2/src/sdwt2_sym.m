function [SPsitLx, Ij, dims_PsitLx, Ncoefs] = sdwt2_sym(x_overlap, I, dims, status, lo, hi, J, M)
...
%     tempd = conv2(in, hi); % extend the signal in a different manner to have another boundary condition: combine wextend and conv(., ., 'valid'), check dimension before that        
    tempa = conv2(wextend('addcol','sym',in,m-1,'b'), lo, 'valid');
    tempd = conv2(wextend('addcol','sym',in,m-1,'b'), hi, 'valid');
...
%     in = conv2(tempa,lo.'); % LL
    PsitLx{1} = conv2(wextend('addrow','sym',tempa,m-1,'b'),hi.','valid'); % LH
    PsitLx{2} = conv2(wextend('addrow','sym',tempd,m-1,'b'),lo.','valid'); % HL
    PsitLx{3} = conv2(wextend('addrow','sym',tempd,m-1,'b'),hi.','valid'); % HH
    in = conv2(wextend('addrow','sym',tempa,m-1,'b'),lo.','valid'); % LL
