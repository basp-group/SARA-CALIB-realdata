...
if ~isfield(param, 'use_nufft_blocks'), param.use_nufft_blocks = 0; end
...
if ~isfield(param, 'use_fft_mask'), param.use_fft_mask = 0; end
...
if param.gen_only_fft_op
    [A, At, ~, S] = op_nufft([0, 0], N, Nn, No, Ns, 0);
    G = [];
    W = [];
    Gw = [];
else
    if ~param.use_nufft_blocks
        ...
        [A, At, Gw, S] = op_nufft(cell2mat(p), N, Nn, No, Ns, 0);
        try
        if ~isempty(param.flag)
            Gw(param.flag<1,:)=0;
            Gw=sparse(Gw);
        end
        end
        ...
        [A, At, Gw, S] = op_nufft([0, 0], N, Nn, No, Ns, 0);
