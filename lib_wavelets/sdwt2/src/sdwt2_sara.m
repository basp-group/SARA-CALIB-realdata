function [SPsitLx] = sdwt2_sara(x_overlap, I, dims, offset, status, J, wavelet)
...
for m = 1:M   
    ...
    if ~strcmp(wavelet{m}, 'self')
        ...
        [SPsitLx_m,~, ~,~] = sdwt2(x_overlap_tmp, I, dims, status, lo, hi, J, M);
    else
        % define the portion of x_overlap to be exploited for the dictionary considered
        for i = 1:dim
            if I(i) > 0
        ...
        SPsitLx_m = x_overlap(temLIdxs(1)+1:end, temLIdxs(2)+1:end)./sqrt(M);
    end
