function [SPsitLx, Ij, dims_PsitLx, Ncoefs] = sdwt2_sara_sym(x_overlap, I, dims, offset, status, J, wavelet)
...
for m = 1:M   
    ...
    if ~strcmp(wavelet{m}, 'self')
        ...
        [SPsitLx_m, Ij((m-1)*(J+1)+1:m*(J+1),:), dims_PsitLx((m-1)*(J+1)+1:m*(J+1),:), Ncoefs((m-1)*(J+1)+1:m*(J+1),:)] = sdwt2_sym(x_overlap_tmp, I, dims, status, lo, hi, J, M);
    else
        % define the portion of x_overlap to be exploited for the dictionary considered
        for i = 1:dim
            if I(i) > 0
        ...
        dims_PsitLx((m-1)*(J+1)+1:m*(J+1),:) = ones(J+1,1)*dims; % see if ok... duplicate otherwise (do not forget to skip a few lines when using dims_PsitLx with 'self')
%         Ncoefs{m} = [];
%         Ij{m} = [];
    end
